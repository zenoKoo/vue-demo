<template>
    <div class="info">
        <div class="info-header">
            <i @click="showInfo" class="icon-back" :style="icons.back"></i>个人中心
        </div>
        <transition name="slide-fade">
            <div class="info-content" v-show="isShowContent">
                <div class="info-resigter">
                    <div class="info-img">
                    ...
                    <div class="info-date">
                        注册于：{{$util.formatDate(user.create_at)}}
                    </div>
                </div>
                <div class="info-collect">
                    ...
                    <div class="collect-content">
                        <div v-for="item of collectTopics" class="collect-item">
                            ...
                            <div class="title">
                                <span @click="view">
                                    <a href="javascript:;" @click="changeRoute('/article?id='+item.id)">{{item.title}}</a>
                                </span>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    export default{
        ...
        computed: {
            userInfo() {
                return this.$store.state.cnode.userInfo
            },
            collectTopics() {
                return this.$store.state.cnode.collectTopics
            },
            ak() {
        ...
        created(){
            ...
            this.$http.get(`https://cnodejs.org/api/v1/topic_collect/${this.userInfo.loginname}`)
                ...
                .then(() => this.$http.get(`https://cnodejs.org/api/v1/user/${this.userInfo.loginname}`))
                    .then(result => result.data.data)

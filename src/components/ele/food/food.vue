<template>
    <div class="food_container">
            ...
            <div class="sort_item" :class="{choose_type:sortBy == 'food'}">
                <div class="sort_item_container" @click="chooseType('food')">
                    <div class="sort_item_border">
                        ...
                        <svg width="10" height="10" version="1.1" class="sort_icon" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <ul>
                                <li v-for="(item, index) in category" :key="index" class="category_left_li" @click="selectCategoryName(item.id, index)" :class="{category_active:restaurant_category_id == item.id}">
                                    ...
                                    <section>
                                        ...
                                        <svg v-if="index" width="8" height="8" version="1.1" xmlns="http://www.w3.org/2000/svg" class="category_arrow">
                                            <path d="M0 0 L6 4 L0 8" stroke-width="1" stroke="#bbb" fill="none"/>
                                        </svg>
                                    </section>
                                </li>
                            </ul>
            ...
            <div class="sort_item" :class="{choose_type:sortBy=='sort'}">
                <div class="sort_item_container" @click="chooseType('sort')">
                    <div class="sort_item_border">
                        ...
                        <svg width="10" height="10" version="1.1" xmlns="http://www.w3.org/2000/svg" class="sort_icon">
                            <polygon points="0,3 10,3 5,8"/>
                        </svg>
                    </div>
                </div>
                    ...
                    <section class="sort_detail_type" v-show="sortBy=='sort'">
                        <ul class="sort_list_container" @click="sortList($event)">
                            <li class="sort_list_li">
                                <svg>
                                    <use xlink:href="#default" xmlns="http://www.w3.org/1999/xlink"/>
                                </svg>
                                <p :class="{sort_select: sortByType == 0}" dat="0">
                                    <span>智能排序</span>
                                    <svg v-if="sortByType == 0">
                                        <use xlink="http://www.w3.org/1999/xlink" xlink:href="#selected"></use>
                                    </svg>
                                </p>
                            </li>
                            <li class="sort_list_li">
                    ...
                    <section class="sort_detail_type filter_container" v-show="sortBy == 'activity'">
                        ...
                        <section style="width:100%;">
                            ...
                            <ul class="filter_ul" style="paddingBottom: .5rem">
                                <li v-for="(item, index) in Activity" :key="item.id" @click="selectSupportIds(index, item.id)" class="filter_li">
                                    <svg v-show="support_ids[index].status" class="activity_svg">
                                        <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#selected"></use>
                                    </svg>
                                    <span :style="{color: '#'+item.icon_color, borderColor: '#'+item.icon_color, borderColor: '#'+item.icon_color}" v-show="!support_ids[index].status" class="filter_icon">{{item.icon_name}}</span>
        ...
        <section class="shop_list_container">
            <shopList :geohash="geohash" :restaurantCategoryId="restaurant_category_id" :restaurantCategoryIds="restaurant_category_ids" :sortByType="sortByType" :deliveryMode="delivery_mode" :confirmSelect="confirmStatus" :supportIds="support_ids" v-if="latitude" @DidConfirm="clearAll"></shopList>
        </section>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            async initData(){
                ...
                this.restaurant_category_id = this.$route.query.restaurant_category_ids
                if(!this.latitude){
            ...
            //选中分类左侧列表的某个选项，右侧渲染相应的列表
            selectCategoryName(id, index){
                //第一个选项 全部商家 点击默认获取所有数据
                if(index == 0){
            ...
            selectDeliveryMode(id){
                //delivery_mode为空 选中当前项 并且filerNum加1
                if(!this.delivery_mode){
                    this.filterNum++
                    this.delivery_mode = id
                //delivery_mode为当前值，清空所选项，filterNum减1

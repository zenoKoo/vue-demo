        ...
        <section class="shop_container" v-if="!showLoading">
            <header class="shop_detail_header" ref="shopheader" :style="{zIndex: showActivities?'14':10}">
                ...
                <section class="description_header">
                    <div class="description_top" @click="toDetail('/shop/shopDetail')">
                        ...
                        <section class="description_right">
                            ...
                            <p class="description_promotion ellipse">公告：{{promotionInfo}}</p>
                        </section>
                        <svg width="14" height="14" xmlns="http://www.w3.org/2000/svg" version="1.1" class="description_arrow">
                            <path d="M0 0 L8 7 L0 14" stroke-width="1" stroke="#fff" fill="none"/>
                        </svg>
                    </div>
                    <footer class="description_footer" v-if="shopDetailData.activities.length" @click="showActivitiesFun">
                        <p class="ellipse">
                            <span class="tip_icon" :style="{background: '#'+shopDetailData.activities[0].icon_color, borderColor: '#'+shopDetailData.activities[0].icon_color}">{{shopDetailData.activities[0].icon_name}}</span>
                            <span>{{shopDetailData.activities[0].description}}（app专享）</span>
                        </p>
                        <p>{{shopDetailData.activities.length}}个活动</p>
                        <svg class="footer_arrow">
                            <use xmlns:link="http://www.w3.org/1999/xlink" xlink:href="#arrow-left"></use>
                        </svg>
                    </footer>
                </section>
            </header>
            <transition name="fade">
                <section class="activities_details" v-if="showActivities">
                    ...
                    <svg width="60" height="60" class="close_activities" @click.stop="showActivitiesFun">
                        <circle r="25" cx="30" cy="30" fill="none" stroke="#555" stroke-width="1"/>
                ...
                <section class="food_container" v-show="changeShowType=='food'">
                    <section class="menu_container">
                        <section class="menu_left" id="wrapper_menu">
                            <ul>
                                <li v-for="(item, index) in menuList" :key="index" class="menu_left_li" @click="chooseMenu(index)" :class="{activity_menu: index == menuIndex}">
                            ...
                            <ul>
                                <li v-for="(item, index) in menuList" :key="index">
                                    ...
                                    <section class="menu_detail_list" v-for="(foods, foodindex) in item.foods" :key="foodindex">
                                        <div class="menu_detail_link" @click="toDetail({path: 'shop/foodDetail', query:{image_path:foods.image_path, description: foods.description, month_sales: foods.month_sales, name: foods.name, rating: foods.rating, rating_count: foods.rating_count, satisfy_rate: foods.satisfy_rate, foods, shopId}})">
                                            ...
                                            <section class="menu_food_description">
                                                <h3 class="food_description_head">
                                                    ...
                                                    <ul class="attributes_ul" v-if="foods.attributes.length">
                                                        <li v-for="(attributes, foodindex) in foods.attributes" :key="foodindex" :style="{color: '#'+attributes.icon_color, borderColor: '#'+attributes.icon_color}" :class="{attribute_new: attributes.icon_name=='新'}">
                                                            <p :style="{color: attributes.icon_name=='新'?'#fff':'#'+attributes.icon_color}">{{attributes.icon_name=='新'?'新品':attributes.icon_name}}</p>
                                                        </li>
                                                    </ul>
                                                </h3>
            ...
            <transition name="fade-choose">
                <section class="rating_container" id="ratingContainer" v-show="changeShowType=='rating'">
                    <section type="2" v-load-more="loadingMoreRating">
                        <section>
    ...
    export default {
        data(){
            return {
                ...
                ratingOffset: 0,//评价获取数据offset的值
                ratingScoresData: 0,//评价总体分数
                ratingTagsList: '',//评价分类列表
                ratingTagsIndex: 0,//评价分类索引
                preventRepeatRequest: false,//阻止多次触发数据请求
                ratingTagName: '',//评价的类型
                ladingRatings: false,//是否显示加载更多评论
                foodScroll: '',//食品列表scroll
                ...
                elBottom: 0,//当前点击骄傲按钮在网页中的绝对bottom值
                ratingScroll: null, //评论员scroll
                wrapperMenu: ''
            }
        },
        created(){
        ...
        computed: {
            ...mapState({
                ...
                cartList: state => state.ele.cartList
            }),
        ...
        methods: {
            ...
            getFoodListHeight(){
                const baseHeight = this.$refs.shopheader.clientHeight,
                      chooseTypeHeight = this.$refs.chooseType.clientHeight,
                      listContainer = this.$refs.menuFoodList;
            ...
            //当滑动食品列表时，监听其scrollTop值来设置相应的食品列表标题的样式
        ...
        .menu_right{
            ...
            .menu_detail_header{
                ...
                .menu_detail_header_right{
                    ...
                    display: block;
                    @include bis('./../../../common/images/ele/icon_point.png');

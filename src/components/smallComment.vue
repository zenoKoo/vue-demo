    ...
    <div>
        ...
        <div v-else>
            ...
            <section class="all-smallCom">
                <div class="smallCom" v-for="item in commentsMsg.comments">
                    <div class="smallCom-rating">
                        <star :score="item.rating.value*2"></star>
                        <span>{{item.created_at}}</span>
                    </div>
                    <div class="smallCom-content">{{item.content}}</div>
                    <div class="smallCom-author">
                        <img class="comment-user-img" :src="item.author.avatar" :alt="item.author.alt">
...
<script type="text/javascript">
    ...
    export default {
        ...
        mounted(){
            const url = '/movie/subject/' + this.$route.params.id + '/comments?apikey=0b2bdeda43b5688921839c8ecb20399b&count=40&client=&udid='
            console.log(url)
...
<style>
  ...
  .smallCom-author .comment-user-img {
    height: 48px;
    width: 48px;
    border-radius: 24px;
  }
